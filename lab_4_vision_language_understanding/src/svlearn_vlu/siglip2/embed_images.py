...
class SIGLIP2Encoder:
    def __init__(self, config):
        """
        Initializes the SIGLIP2Encoder with the SIGLIP2 model, processor, and Qdrant client.
        
        Args:
            config (dict): Configuration dictionary containing model, database, and dataset paths.
        """
        # Load SIGLIP2 model and processor
        self.model_name = config["siglip2"]["model"]
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.device = torch.device("mps") if torch.backends.mps.is_available() else self.device
        self.processor = AutoProcessor.from_pretrained(self.model_name)
        self.model = AutoModel.from_pretrained(self.model_name).to(self.device)

        # Get the vector dimension (handle BaseModelOutputWithPooling in newer transformers)
        _out = self.model.get_image_features(**self.processor(images=[Image.new("RGB", (224, 224))], return_tensors="pt").to(self.device))
        if isinstance(_out, torch.Tensor):
            _emb = _out
        else:
            _pooler = getattr(_out, "pooler_output", None)
            _emb = _pooler if _pooler is not None else _out.last_hidden_state[:, 0]
        self.vector_dim = _emb.shape[-1]
        # Connect to Qdrant
        start_vector_db()
        self.client = QdrantClient(url=f"http://localhost:{PORT}")

        # Initialize collection name
        self.collection_name = config["siglip2"]["collection_name"]
    ...
    def encode_images(self, image_dir):
        ...
        for idx, img_path in enumerate(tqdm(image_paths, desc="Encoding images", unit="image")):
            ...
            # Generate image embedding (handle BaseModelOutputWithPooling in newer transformers)
            with torch.no_grad():
                ...
                if isinstance(out, torch.Tensor):
                    ...
                    embedding = pooler if pooler is not None else out.last_hidden_state[:, 0]
...
if __name__ == "__main__":
    # Initialize SIGLIP2Encoder
    siglip2_encoder = SIGLIP2Encoder(config)

    # Run the pipeline
    siglip2_encoder.run(config["datasets"]["trees"])